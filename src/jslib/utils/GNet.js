...
export function call(...args) {
  ...
  param.header["Cookie"] = wx.getStorageSync("cookie")
    ? `openid=${wx.getStorageSync("cookie")}`
    : "";
  if (param.params) {
  ...
  if (
    _httpInfo.hasOwnProperty("credentials") &&
    !param.hasOwnProperty("credentials")
  ) {
  ...
  function onResult(d, resolve, reject) {
    ...
    if (util.onMode("debug")) {
      console.log(
        "%c[Received] " + param.url.replace(_serverPath, ""),
        "color:#3366dd",
        dObj
      );
    }
  ...
  function onError(d, reject) {
    if (util.onMode("debug")) {
      console.log(
        "[ERROR]" + param.url.replace(_serverPath, ""),
        "status:",
        d.status,
        d
      );
  ...
  var reqKey =
    "" + param.url + param.method + JSON.stringify(param.params || {});
  if (
    !_reqHash[reqKey] ||
    (_reqHash[reqKey] &&
      new Date().getTime() - _reqHash[reqKey].time >=
        (_httpInfo.repeatReqTime || 2000))
  ) {
