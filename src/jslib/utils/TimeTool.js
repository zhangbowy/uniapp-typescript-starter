...
export function getFullDateArray($time, $debit) {
  return getDateArray($time, $debit).concat(getTimeArray($time, $debit));
}
...
export function getTime($time, $debit) {
  return getTimeArray($time, $debit).join(":");
}
...
function getBasicDateArray(da, debit) {
  return [
    debit
      ? da.getFullYear() < 10
        ? "0" + da.getFullYear()
        : da.getFullYear().toString()
      : da.getFullYear(),
    debit
      ? da.getMonth() + 1 < 10
        ? "0" + (da.getMonth() + 1)
        : (da.getMonth() + 1).toString()
      : da.getMonth() + 1,
    debit
      ? da.getDate() < 10
        ? "0" + da.getDate()
...
function getBasicTimeArray(da, debit) {
  return [
    debit
      ? da.getHours() < 10
        ? "0" + da.getHours()
        : da.getHours().toString()
      : da.getHours(),
    debit
      ? da.getMinutes() < 10
        ? "0" + da.getMinutes()
        : da.getMinutes().toString()
      : da.getMinutes(),
    debit
      ? da.getSeconds() < 10
        ? "0" + da.getSeconds()
        : da.getSeconds().toString()
      : da.getSeconds(),
  ];
}
...
export function getCountDown($time, type = TYPE_HMS, debit = false) {
  ...
  if (type.indexOf(TYPE_H) >= 0) {
    if (type == TYPE_HMS) {
      //如果格式是hh:mm:ss 那么 hh位应该显示真实的小时数，而不是按一天的秒数取余再算小时数
...
export function getPastSecond($time = 0) {
  ...
  return (
    Math.floor(Number(temp[0])) * 3600 +
    Math.floor(Number(temp[1])) * 60 +
    Math.floor(Number(temp[2]))
  );
}
...
export function getMonthByOffset(monthOffset = 0, sundayFirst = true) {
  ...
  resultObj.startTime =
    new Date(startDateArr[0], startDateArr[1], startDateArr[2]).getTime() /
    1000;
  resultObj.endTime =
    new Date(endDateArr[0], endDateArr[1], endDateArr[2]).getTime() / 1000 +
    86400;
